...
api_base_url2 = "https://inkar-suki.codethink.cn"
...
async def jx3api_request2(url):
    print("daoda1")
    async with httpx.AsyncClient() as client:
        print(api_base_url2 + url)
        response = await client.get(api_base_url2 + url)
        #print(response)
    return response.json()
...
jx3api_home_flower = on_command(head + "交易行")
...
async def handle_jx3api_home_flower(event, args: Message = CommandArg()):
    ...
    if server is None:
       ...
       await jx3api_home_flower.finish("请输入要查询的物品")



    response = await jx3api_request2("/trade?server="+server+"&name="+name+"&token=119032235")  # 修改API的endpoint
    #print(response)
    if response["code"] == 200:
        #print(response['data'][1]['price_str'])
        text =""
        for x in response['data']:
          text=text+MessageSegment.image(x['icon'])+"  "+x['name']+"  "+x['time_str']+"  "+x['price_str']+"\n"
        print(text)
        await jx3api_home_flower.finish(text)
        #await jx3api_home_flower.finish(MessageSegment.image(response['data'][1]['icon'])+"\n"+response['data'][1]['price_str'])
...
async def handle_web_announce():
    ...
    print(params)
    response = await jx3api_request("/data/web/news/announce", params)  # 修改API的endpoint
    print(response)
    if response["code"] == 200:
        todaydate=response['data']
        print(todaydate[1]['url'])
        await jx3api_web_announce.finish(MessageSegment.image(todaydate[1]['url']))
    else:
